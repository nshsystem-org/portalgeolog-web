...
async function verifyPhoneNumber() {
  ...
  try {
    ...
    if (data.error) {
      ...
      console.log('   https://business.facebook.com/settings/whatsapp-business-accounts');
      return false;
    }
...
async function verifyBusinessAccount() {
  ...
  try {
    ...
    if (data.error) {
      console.error('❌ Erro ao verificar Business Account:', data.error.message);
      console.error('   Código:', data.error.code);
      console.error('   Subcódigo:', data.error.error_subcode);
      console.log('');
      console.log('💡 Dica: O WABA ID está na URL ao acessar:');
      console.log('   https://business.facebook.com/wa/manage/phone-numbers/?waba_id=XXXXXXXX');
      return false;
    }
...
(async () => {
  ...
  if (!phoneOk || !businessOk) {
    ...
    console.log('\n📖 Guia de correção:');
    console.log('1. Acesse: https://business.facebook.com/settings/whatsapp-business-accounts');
  ...
  if (messageOk) {
    console.log('🎉 Todas as verificações passaram! A API está configurada corretamente.');
  } else {
    console.log('\n⚠️  As credenciais estão corretas, mas o envio falhou.');
    console.log('   Verifique se o número de destino está registrado ou se há limites de envio.');
  }
})();
