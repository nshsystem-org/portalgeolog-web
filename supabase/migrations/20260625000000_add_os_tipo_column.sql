-- =============================================================================
-- Migration: add_os_tipo_column
-- Data: 2026-06-25
-- =============================================================================
-- Objetivo: Suportar os tipos de atendimento OS, Freelance e Rascunho de forma
--           unificada, substituindo a flag booleana is_freelance por uma coluna
--           enum 'tipo' (não-destrutivo para a tabela sagrada ordens_servico).
--
-- Alterações:
--   1. ordens_servico → ADD COLUMN tipo TEXT DEFAULT 'os' (não-destrutivo)
--   2. Migra dados existentes: is_freelance=true vira tipo='freelance'
--   3. insert_os_atomic → lê 'tipo' do payload e persiste; mantém is_freelance
--      sincronizado para compatibilidade com código legado.
--   4. update_os_atomic → preserva tipo existente se não informado; sincroniza
--      is_freelance conforme o tipo.
-- =============================================================================
...
-- ── 2. Migrar registros existentes para não perder a classificação freelance ──
UPDATE public.ordens_servico
  SET tipo = 'freelance'
  WHERE is_freelance = true AND tipo = 'os';

-- ── 3. Garantir valores válidos na coluna (não-destrutivo) ──────────────────
ALTER TABLE public.ordens_servico
  DROP CONSTRAINT IF EXISTS chk_ordens_servico_tipo;

ALTER TABLE public.ordens_servico
  ADD CONSTRAINT chk_ordens_servico_tipo
    CHECK (tipo IN ('os', 'freelance', 'rascunho'));

-- ── 4. insert_os_atomic: inclui 'tipo' no INSERT e sincroniza is_freelance ───
...
BEGIN
  ...
  -- tipo: default 'os' se não informado; sincroniza is_freelance para compatibilidade
...
-- ── 5. update_os_atomic: preserva tipo se não informado; sincroniza is_freelance ─
...
BEGIN
  ...
  -- tipo: preserva o valor atual da linha se não informado no payload
  v_tipo := COALESCE(NULLIF(p_os_data->>'tipo', ''), tipo);
  ...
  SET
    ...
    -- tipo: só atualiza se explicitamente presente no payload; sincroniza is_freelance
    tipo              = v_tipo,
