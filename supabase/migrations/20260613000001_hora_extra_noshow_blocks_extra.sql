-- No-show bloqueia hora extra e reduz repasse do motorista pelo mesmo percentual.
-- Regras:
--   - Se no_show = true: hora_extra zerada, repasse = custo * no_show_percentual/100
--   - Se no_show = false: hora_extra calculada normalmente, repasse = custo + extra_motorista
...
BEGIN
  ...
  -- No-show bloqueia hora extra
  IF v_no_show THEN
    v_hora_extra_cliente   := 0;
    v_hora_extra_motorista := 0;
  ELSE
  ...
  IF v_no_show THEN
    v_base_cobranca   := v_v_bruto * v_no_show_fator;
    v_repasse_efetivo := v_v_custo * v_no_show_fator;
  ELSE
  ...
  INSERT INTO public.ordens_servico (
    ...
    v_imposto, v_v_custo, v_lucro,
    'Pendente', 'Pendente', p_actor_id, p_actor_name
  )
  ...
  IF p_waypoints IS NOT NULL AND jsonb_typeof(p_waypoints) = 'array' THEN
    ...
    LOOP
      ...
      IF COALESCE(v_wp->>'comment', '') <> '' THEN
        INSERT INTO public.os_waypoint_comments (ordem_servico_id, waypoint_position, waypoint_label, comment)
        VALUES (v_os_id, v_position, COALESCE(v_wp->>'label', ''), v_wp->>'comment');
      END IF;
...
BEGIN
  ...
  IF p_waypoints IS NOT NULL AND jsonb_typeof(p_waypoints) = 'array' AND jsonb_array_length(p_waypoints) > 0 THEN
    ...
    INSERT INTO public.os_waypoint_passengers (waypoint_id, passageiro_id)
    SELECT w.id, (pass->>'solicitante_id')::UUID
    ...
    JOIN public.os_waypoints w
      ON w.ordem_servico_id = p_os_id AND w.position = (wp_idx - 1)::integer;

    INSERT INTO public.os_waypoint_comments (ordem_servico_id, waypoint_position, waypoint_label, comment)
    SELECT p_os_id, w.position, COALESCE(wp.value->>'label', ''), wp.value->>'comment'
    FROM jsonb_array_elements(p_waypoints) WITH ORDINALITY AS wp(value, wp_idx)
    JOIN public.os_waypoints w
      ON w.ordem_servico_id = p_os_id AND w.position = (wp_idx - 1)::integer
