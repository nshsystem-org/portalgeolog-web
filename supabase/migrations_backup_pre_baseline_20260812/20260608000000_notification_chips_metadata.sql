-- Add metadata column to app_notifications for structured chip data
alter table public.app_notifications
  add column if not exists metadata jsonb default '{}';

comment on column public.app_notifications.metadata is 'Dados estruturados da notificação (ex: lista de campos alterados para chips).';

-- Update trigger to generate clean chip-style messages and store field list in metadata
...
begin
  ...
  if NEW.type = 'update'
    ...
    -- Build the old pipe-separated message for backwards compatibility (unused now)
    ...
    -- Build clean chip labels as JSONB array
    select jsonb_agg(distinct label order by sort_order)
  ...
  case NEW.type
    when 'update' then
      ...
      if v_changed_fields_list is not null then
        v_message := format(
          'OS %s: Atualizações realizadas.',
  ...
  values (
    ...
    NEW.actor_name,
    v_avatar_url,
    jsonb_build_object('changed_fields_list', v_changed_fields_list)
  );
