-- Migration: update_os_atomic agora sincroniza status_operacional após
-- recriar os ciclos operacionais.
--
-- Bug: Ao editar uma OS (executeSaveOS → updateOSInDB → update_os_atomic),
-- a RPC faz DELETE + INSERT dos ciclos mas NÃO atualiza status_operacional.
-- Isso cria divergência entre o status da coluna e o estado real dos ciclos.
-- Exemplo observado em produção:
--   - OS 2026061229: status_operacional='Pendente', ciclos=[completed, completed]
--   - OS 2026061135: status_operacional='Finalizado', ciclos=[pending]
--
-- Correção: após recriar os ciclos, derivar o status operacional usando
-- derive_os_operational_status_from_cycles e atualizar a coluna na mesma
-- transação atômica.

DROP FUNCTION IF EXISTS public.update_os_atomic(uuid, jsonb, jsonb, jsonb);
DROP FUNCTION IF EXISTS public.update_os_atomic(uuid, jsonb, jsonb, jsonb, jsonb);
...
BEGIN
  ...
  -- Sincronizar status_operacional com base nos ciclos recém-persistidos.
  -- Garante consistência entre a coluna e o estado real dos ciclos, evitando
  -- que o status fique "Pendente" quando todos os ciclos estão "completed",
  -- ou "Finalizado" quando os ciclos foram resetados para "pending".
